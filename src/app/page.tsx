...
export default function Home() {
  ...
  const loadProgress = useCallback(async (accessHash: string) => {
    try {
      const userProgress: { module: string; questions_attempted: number; questions_correct: number; streak_days: number }[] = await getAllProgress(accessHash);
  ...
  const handleCheckAnswer = useCallback((answers: Record<string, string>): boolean => {
    ...
    return correct;
  }, [currentQuestion, user, loadProgress, accessHash]);
